    ...
    <View className="bg-solar-blue-primary flex-1">
      ...
      <View className="p-4 bg-white rounded-t-3xl flex-1">
        <Card className="border border-gray-300 bg-white shadow-sm shadow-slate-950">
          ...
          <CardContent>
            <Text className="text-center text-3xl font-bold">R$ {maskMoney((parseFloat(totalAmount as string).toFixed(2)))}</Text>
          </CardContent>
        </Card>

        <View className="py-4 mt-4">
          <Text className="text-xl font-bold text-solar-blue-secondary text-center">Escolha uma forma de pagamento</Text>
        </View>

        <View className="flex-row justify-between gap-4">
          <TouchableOpacity onPress={pixPaymentMethod} className="w-48 border border-gray-300 bg-white shadow-sm shadow-slate-950 rounded-lg">
            <Card className='border-0'>
              <CardTitle className='text-center p-4 text-lg font-medium text-gray-600'>
                PIX
              </CardTitle>
              <CardContent className="items-center justify-center">
                <Image source={require("@/assets/images/pix.png")} className="w-20 h-20" />
              </CardContent>
            </Card>
          </TouchableOpacity>
          <TouchableOpacity 
          onPress={() => router.push({
            ...
            params: { dataOrder: JSON.stringify(order), totalAmount: totalAmount}
          })}
