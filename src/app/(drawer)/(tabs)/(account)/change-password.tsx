...
const MyAccount = () => {
  ...
  const { control, handleSubmit, reset, formState: { errors } } = useForm<AlterPasswordFormType>({
    defaultValues: {},
    resolver: zodResolver(AlterPasswordSchema)
  });

  const onSubmit: SubmitHandler<AlterPasswordFormType> = async (data: AlterPasswordFormType) => {
            ...
            <View className='p-4 bg-white rounded-t-3xl h-full'>
              <View className='flex-col gap-4 my-4'>
                ...
                {errors.senhaAnterior && (
                  <Text className='text-solar-red-primary -mt-4'>{errors.senhaAnterior?.message}</Text>
                )}
